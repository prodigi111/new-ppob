...
const SITE_LOGO = theme.assets.logo;

export const Footer = () => {
  const { store, isReseller } = useStore();
  const storeName = isReseller ? store?.store_name || 'Store' : theme.brand.name;
  const logoSrc = isReseller && store?.logo_url ? imgUrl(store.logo_url) : SITE_LOGO;
  return (
